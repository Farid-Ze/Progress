...
exports.getRegionalImpactAnalytics = exports.updatePartner = exports.getPartnerDashboard = exports.createPartnerRegistration = exports.getKatalisatorInitiatives = exports.getKatalisatorPartners = exports.getRegionalStats = void 0;
...
const getRegionalStats = async (req, res) => {
    try {
        ...
        if (region && typeof region === 'string') {
            stats = stats.filter(stat => stat.region.toLowerCase().includes(region.toLowerCase()));
        }
        const totalStats = {
...
exports.getRegionalStats = getRegionalStats;
/**
 * Get Katalisator partners
 */
const getKatalisatorPartners = async (req, res) => {
    try {
        const { region, type, status, page = 1, limit = 10 } = req.query;
        let partners = mockPartners;
        // Filter by region
        if (region && typeof region === 'string') {
            partners = partners.filter(partner => partner.region.toLowerCase().includes(region.toLowerCase()));
        }
        // Filter by type
...
exports.getKatalisatorPartners = getKatalisatorPartners;
/**
 * Get Katalisator initiatives
 */
const getKatalisatorInitiatives = async (req, res) => {
    try {
        const { region, status, category, page = 1, limit = 10 } = req.query;
        let initiatives = mockInitiatives;
        // Filter by region
        if (region && typeof region === 'string') {
            initiatives = initiatives.filter(init => init.region.toLowerCase().includes(region.toLowerCase()));
        }
        // Filter by status
        if (status && typeof status === 'string') {
...
exports.getKatalisatorInitiatives = getKatalisatorInitiatives;
/**
 * Create new partner registration
 */
const createPartnerRegistration = async (req, res) => {
    try {
        const { name, type, region, contactInfo, description, documents } = req.body;
        // Validation
...
exports.createPartnerRegistration = createPartnerRegistration;
/**
...
const getPartnerDashboard = async (req, res) => {
    try {
        ...
        const partnerInitiatives = mockInitiatives.filter(init => init.partner.id === partnerId);
        // Calculate metrics
        const metrics = {
 ...
 */
const getRegionalImpactAnalytics = async (req, res) => {
    try {
        const { region } = req.params;
        const { period = 'ytd' } = req.query;
        const regionStats = mockRegionalStats.find(stat => stat.region.toLowerCase() === region.toLowerCase());
        if (!regionStats) {
            return res.status(404).json({
                success: false,
        ...
        const regionInitiatives = mockInitiatives.filter(init => init.region.toLowerCase() === region.toLowerCase());
...
exports.getRegionalImpactAnalytics = getRegionalImpactAnalytics;
//# sourceMappingURL=katalisatorController.js.map