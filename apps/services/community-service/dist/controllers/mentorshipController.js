...
exports.getMentorshipAnalytics = exports.addSessionFeedback = exports.getMentorshipSessions = exports.scheduleMentorshipSession = exports.respondToMentorshipRequest = exports.getUserMentorshipRequests = exports.createMentorshipRequest = exports.getMentorById = exports.getAllMentors = void 0;
// Mock data
const mockMentors = [
    {
        id: '1',
...
const getAllMentors = async (req, res) => {
    try {
        const { expertise, industry, experience, pricing, rating, availability, search, sortBy = 'rating', sortOrder = 'desc', page = 1, limit = 12 } = req.query;
        let mentors = [...mockMentors].filter(mentor => mentor.isActive);
        // Filter by expertise
        if (expertise && typeof expertise === 'string') {
            mentors = mentors.filter(mentor => mentor.expertise.some(exp => exp.toLowerCase().includes(expertise.toLowerCase())));
        }
        // Filter by industry
        if (industry && typeof industry === 'string') {
            mentors = mentors.filter(mentor => mentor.industries.some(ind => ind.toLowerCase().includes(industry.toLowerCase())));
        }
        ...
        if (availability === 'available') {
            mentors = mentors.filter(mentor => mentor.activeMentees < mentor.availability.maxMenteesPerMonth);
        }
        // Search in name, bio, expertise
        if (search && typeof search === 'string') {
            const searchTerm = search.toLowerCase();
            mentors = mentors.filter(mentor => mentor.name.toLowerCase().includes(searchTerm) ||
...
const getMentorById = async (req, res) => {
    try {
        ...
        if (userId) {
            activeMentorship = mockMentorshipRequests.find(r => r.mentorId === mentorId &&
                r.menteeId === userId &&
...
const createMentorshipRequest = async (req, res) => {
    try {
        const { mentorId } = req.params;
        const { topic, description, goals, preferredDuration, preferredFrequency, urgency = 'medium' } = req.body;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const existingRequest = mockMentorshipRequests.find(r => r.mentorId === mentorId &&
            r.menteeId === userId &&
            ['pending', 'accepted'].includes(r.status));
        if (existingRequest) {
 ...
 */
const getUserMentorshipRequests = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
...
exports.getUserMentorshipRequests = getUserMentorshipRequests;
/**
...
const scheduleMentorshipSession = async (req, res) => {
    try {
        const { requestId } = req.params;
        const { title, description, scheduledAt, duration = 60, type = 'video-call', agenda = [] } = req.body;
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
                success: false,
...
exports.scheduleMentorshipSession = scheduleMentorshipSession;
/**
...
exports.getMentorshipSessions = getMentorshipSessions;
/**
...
exports.addSessionFeedback = addSessionFeedback;
/**
 * Get mentorship analytics
 */
const getMentorshipAnalytics = async (req, res) => {
    try {
...
exports.getMentorshipAnalytics = getMentorshipAnalytics;
