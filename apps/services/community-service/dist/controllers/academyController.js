...
exports.searchCourses = exports.getPopularCourses = exports.getCourseAnalytics = exports.updateLessonProgress = exports.getUserEnrollments = exports.enrollInCourse = exports.getCourseById = exports.getAllCourses = void 0;
// Mock data
const mockCourses = [
    {
        id: '1',
...
const getAllCourses = async (req, res) => {
    try {
        const { category, level, search, instructor, sortBy = 'createdAt', sortOrder = 'desc', page = 1, limit = 12 } = req.query;
        ...
        if (instructor && typeof instructor === 'string') {
            courses = courses.filter(course => course.instructor.name.toLowerCase().includes(instructor.toLowerCase()));
        }
...
const getCourseById = async (req, res) => {
    try {
        ...
        let enrollment = null;
        if (userId) {
            enrollment = mockEnrollments.find(e => e.courseId === courseId && e.userId === userId);
        }
...
const enrollInCourse = async (req, res) => {
    try {
        ...
        const existingEnrollment = mockEnrollments.find(e => e.courseId === courseId && e.userId === userId);
...
const updateLessonProgress = async (req, res) => {
    try {
        ...
        const enrollment = mockEnrollments.find(e => e.courseId === courseId && e.userId === userId);
        if (!enrollment) {
            return res.status(404).json({
                success: false,
...
const searchCourses = async (req, res) => {
    try {
        ...
        category, level, instructor, minRating, maxPrice, minDuration, maxDuration, hascertificate, language, sortBy = 'relevance', page = 1, limit = 12 } = req.query;
        let courses = [...mockCourses].filter(course => course.isActive);
        ...
        if (maxPrice)
            courses = courses.filter(c => c.price <= parseFloat(maxPrice));
        if (minDuration)
            courses = courses.filter(c => c.duration >= parseInt(minDuration));
        if (maxDuration)
            courses = courses.filter(c => c.duration <= parseInt(maxDuration));
        if (hascertificate === 'true')
            courses = courses.filter(c => c.certificate);
        if (language)
