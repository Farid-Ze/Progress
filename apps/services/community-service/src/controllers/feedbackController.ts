...
interface FeedbackResponse {
  ...
  content: string;
  type: 'comment' | 'status_update' | 'solution';
  createdAt: Date;
}
 ...
 */
export const addFeedbackResponse = async (req: Request, res: Response) => {
  try {
    const { feedbackId } = req.params;
    ...
    const response: FeedbackResponse = {
      ...
      author: {
        id: userId || 'anonymous',
        name: userName,
        role: (userRole as 'user' | 'admin' | 'developer') || 'user'
      },
      content,
      type,
      createdAt: new Date()
 ...
 */
export const getTrendingFeedback = async (req: Request, res: Response) => {
  try {
